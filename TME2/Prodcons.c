...
pthread_mutex_t cptmut=PTHREAD_MUTEX_INITIALIZER;
...
t_messager	make_messager(int ref,fifo *tapis_prod,fifo *tapis_cons,ft_scheduler_t *schedtprod,ft_scheduler_t *schedtcons)
{
	...
	m.schedcons=schedtcons;
	m.schedprod=schedtprod;
...
t_messager	void_to_messager(void *conso)
{
	...
	m.schedcons=tmp->schedcons;
	m.schedprod=tmp->schedprod;
	m.paquet=tmp->paquet;
	return m;
}
...
t_consommateur	void_to_consommateur(void *conso)
{
	...
	c.f = tmp->f;
	c.ref = tmp->ref;
	p.schedtcons = tmp->schedtcons;
	return c;
}

void _producteur(void *conso)
{
